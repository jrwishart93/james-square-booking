  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        if (!agreedToTerms) {
          setTermsError('You must accept the Terms of Use, Data Protection, and Code of Conduct to continue.');
          return;
        }
        if (!residentType) {
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {isRegistering && (
          <>
            ...
            <div className="mt-4">
              <label className="flex items-start gap-3 rounded-lg border border-gray-200 dark:border-gray-700 px-3 py-3">
                <input
                  type="checkbox"
                  ...
                  required
                />
                <span className="text-sm text-gray-900 dark:text-gray-100 leading-5">
                  I agree to the{' '}
                  <button
                    type="button"
                    onClick={() => setShowTermsModal(true)}
                    className="text-blue-700 dark:text-blue-300 underline hover:no-underline focus:outline-none focus:ring-2 focus:ring-blue-500 rounded"
                  >
